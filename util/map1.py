...
class Treasure:
    ...
    def FindTreasure(self, img, output_mode=0):
        """
        找宝藏
        @param img: 输入图像
        @param output_mode: 0为输出宝藏的像素坐标（浮点数） 1为输出宝藏的方块坐标（整数）
        Args:
            img: 输入图像
            ...
            if len(cnt) > 5:  # 点的数量够多，滤噪点
                ...
                if (min < S1 < max) and \
                    ...
                    if output_mode == 0:
                        ...
                        location.append((int(ell[0][0] // int(length/10)) + 1, int(ell[0][1] // int(height/10)) + 1))
                        # location中的点坐标第一个是第几列，第二个是第几行
